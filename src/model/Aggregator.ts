...
export default class Aggregator {
	...
	private min(data: AggregateData): number {
		const values = this.toValues(this.rejectNaNs(data));
		return Number(R.reduce(R.min, Number.MAX_VALUE, values));
	}

	private max(data: AggregateData): number {
		const values = this.toValues(this.rejectNaNs(data));
		return Number(R.reduce(R.max, Number.MIN_VALUE, values));
	}
