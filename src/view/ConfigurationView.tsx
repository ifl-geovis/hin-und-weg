...
export default class App extends React.Component<IConfigurationProps, IConfigurationState> {
    ...
    public render(): JSX.Element {
        ...
        if (this.props.geodata) {
            geoFieldOptions = R.map((field) =>  {
                    return { label: field, value: field };
            } , this.props.geodata.fields());
        }
        ...
        const tablesfiles = R.map((tablefile) => {
            return (<div key={tablefile} className="p-col-12">{tablefile}</div>);
        ...
        return (
            <div>
                <Panel header="1. Geodaten">
                    ...
                    {geodatafile}
                    <Dropdown className="p-col-6" key="geoId"
                              value={this.state.geoId} options={geoFieldOptions} disabled={this.props.geodata == null}
                              placeholder="ID Spalte auswählen"
                              onChange={(e) => {this.setState({geoId: e.value}); } }/>
                    <Dropdown className="p-col-6" key="geoName"
                              value={this.state.geoName} options={geoFieldOptions} disabled={this.props.geodata == null}
                              placeholder="Namenspalte auswählen"
                              onChange={(e) => {this.setState({geoName: e.value}); } }/>
                    </div>
                </Panel>
