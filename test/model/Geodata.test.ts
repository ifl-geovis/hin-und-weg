...
describe("Load shapefile and provide geojson data", () => {
  ...
  it("Iterate trough data and check values (geometry and properties)", done => {
    Geodata.read("./testdata/ot.shp", (data: Geodata) => {
      for (const index of R.range(0, data.count())) {
  ...
  it("Transformes geodata into WGS 84 projection", done => {
    Geodata.read("./testdata/ot.shp", (data: Geodata) => {
      const polygon = data.getFeatureOf(0).geometry as Polygon;
      expect(polygon.coordinates[0][0]).to.deep.equal([
        317483.3169999972,
        5691380.912100001,
      ]);
      const wgs84Data = data.transformToWGS84();
      expect(wgs84Data.projection).to.deep.equal(Proj4.WGS84);
      const polygonWGS84 = wgs84Data.getFeatureOf(0).geometry as Polygon;
      expect(wgs84Data.transformToWGS84().projection).to.deep.equal(
        Proj4.WGS84,
      );
      done();
    });
